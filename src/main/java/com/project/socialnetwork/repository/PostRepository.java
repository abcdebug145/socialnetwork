...
public interface PostRepository extends JpaRepository<Post, Long> {
    ...
    @Query(value = "SELECT * FROM posts ORDER BY RAND()", nativeQuery = true)
    java.util.List<Post> getRandomPosts();

    @Query(value = "SELECT p.* FROM posts p JOIN accounts a ON p.account_id = a.id WHERE CONCAT(p.title, a.username, p.content) LIKE CONCAT('%', :keyword, '%')", nativeQuery = true)
